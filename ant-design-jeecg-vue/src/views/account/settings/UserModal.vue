    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="用户账号"  :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入用户账号" :disabled="true" v-decorator="[ 'username', validatorRules.username]" :readOnly="!!model.id"/>
        </a-form-item>

        <template v-if="!model.id">
          <a-form-item label="登陆密码" :labelCol="labelCol" :wrapperCol="wrapperCol" >
            <a-input type="password" :disabled="true"  placeholder="请输入登陆密码" v-decorator="[ 'password', validatorRules.password]" />
          </a-form-item>

          <a-form-item label="确认密码" :labelCol="labelCol" :wrapperCol="wrapperCol" >
            <a-input type="password"  :disabled="true" @blur="handleConfirmBlur" placeholder="请重新输入登陆密码" v-decorator="[ 'confirmpassword', validatorRules.confirmpassword]"/>
          </a-form-item>
        </template>

        <a-form-item label="用户名字" :labelCol="labelCol" :wrapperCol="wrapperCol" >
          <a-input placeholder="请输入用户名称"  :disabled="true" v-decorator="[ 'realname', validatorRules.realname]" />
        </a-form-item>
        ...
        <a-form-item label="部门分配" :labelCol="labelCol" :wrapperCol="wrapperCol" >
          <a-input-search
            ...
            disabled
            @search="onSearch">
            <a-button slot="enterButton" :disabled="true"  icon="search">选择</a-button>
          </a-input-search>
        </a-form-item>
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            obj.then((res)=>{
              if(res.success){
                Vue.ls.set(USER_INFO, formData, 7 * 24 * 60 * 60 * 1000)
                that.$store.commit('SET_INFO', formData);
